...
Write-Host ("重启 dsh：cd {0}; pnpm start" -f $QuotedRoot)
Write-Host ''
# Single-quoted template + concatenation: the printed command contains `$s` and `$env:TEMP`, and a
# double-quoted string would evaluate them here instead of printing them.
$UpdateCommand = if ($InstallMode -eq 'managed') {
  '$s="$env:TEMP\dsh-install.ps1"; irm ' +
    ('https://raw.githubusercontent.com/{0}/refs/heads/{1}/scripts/install.ps1' -f $Repository, $RemoteRef) +
    ' -OutFile $s; powershell -NoProfile -ExecutionPolicy Bypass -File $s'
} else {
  '.\scripts\install.ps1'
}
Write-Pair '以后更新：重跑同一条安装命令，再在 Chrome 点一次「重新加载」。' 'Later updates: rerun the same install command, then one more “Reload” in Chrome.'
Write-Host ("   {0}" -f $UpdateCommand)
Write-Host ''
Write-Pair "本仓库 fork 自 omdsh-dev/dsh-browser（原始仓库），支持请到：https://github.com/omdsh-dev/dsh-browser" "This repository is a fork of omdsh-dev/dsh-browser (the original project) — support it there: https://github.com/omdsh-dev/dsh-browser"
